...
Curated rolling updates for the in-app changelog view.
...
- Snooze countdown is shown in the status line, while the main alarm button shows the next phase.
- Reset now clears the session cycle; Stop still just stops the timer.
- Idle alarm auto-end now defaults to 30 minutes.

## Next Up
- Session controls redesign (New / Stop / Reset / Next).
- Stale-session boundary and recovery flow.
